...
pub fn render_text(text: &String, pos: Vec2, color: Color, align: TextAlign, texture: &Texture2D) {
    let (change, iter): (f32, Box<dyn Iterator<Item=_>>) = match align {
        TextAlign::Left  => ( CHAR_WIDTH, Box::new(text.chars())),
        TextAlign::Right => (-CHAR_WIDTH, Box::new(text.chars().rev())),
    };
    ...
    for c in iter {
        ...
        let rect = Rect {
            x: CHARS_ORIGIN.x + c_pos.0 as f32 * CHAR_WIDTH,
            y: CHARS_ORIGIN.y + c_pos.1 as f32 * CHAR_HEIGHT,
            w: CHAR_WIDTH,
            h: CHAR_HEIGHT,
        };
